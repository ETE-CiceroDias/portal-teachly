      ...
      <div className="aula-row" onClick={() => !editMode && setOpen(o => !o)}>
            ...
            <>
              ...
              {st.data_aula && (
                <div className="aula-meta aula-date-tag">
                  📅 {st.data_aula}
                  {st.data_aula_fim && st.data_aula_fim !== st.data_aula && (
                    <> → {st.data_aula_fim}</>
                  )}
                </div>
              )}
        ...
        {editMode ? (
          <div style={{ display:'flex', gap:4, marginLeft:8 }} onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={() => setConfirm(true)} title="Remover aula" style={{
              background:'none', border:'none', cursor:'pointer', color:'var(--red)', fontSize:'0.85rem',
            }}>🗑</button>
            {confirm && (
              <ConfirmModal
                title="Remover aula"
                message={"Tem certeza que deseja remover a aula \"" + (aula.titulo?.split('\n')[0] || '') + "\"? Esta ação não pode ser desfeita."}
                confirmLabel="Remover"
                onConfirm={() => { setConfirm(false); onDeleteAula(); }}
...
export function Bloco({ bloco, blocoIdx, courseKey, turmaKey, state, onToggle, onSave, onReorder,
  ...
  const [confirmBloco, setConfirmBloco] = useState(false);
